...
* **API Key / Token Authentication**: Configurable security middleware (`src/api/auth.py`) that enforces `X-API-Key` validation on protected API routes (`POST /api/query`, `POST /api/ingest`, `PUT /api/documents/tags`). Operates seamlessly in zero-config Public mode when `CHRONOS_API_KEY` is empty, or enforces key verification when set. Includes frontend client key persistence in localStorage.
...
* **Automated RAG Evaluation Benchmark Suite (RAGAS Metrics)**: Quantitative evaluator (`src/generation/evaluator.py`) that computes 4 core RAGAS metrics for every query response: **Faithfulness** (zero hallucination score), **Answer Relevance** (direct query match), **Context Precision** (retrieved signal-to-noise ratio), and **Context Recall** (completeness). Displays an overall RAGAS benchmark score badge and progress bar breakdown panel in the UI.
...
* **`DELETE /api/cache`**: Manually flush all cached query responses.
* **`GET /api/cache/stats`**: Retrieve telemetry stats (active entries, capacity, TTL) for the query cache.
